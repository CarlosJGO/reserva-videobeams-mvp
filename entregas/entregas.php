...
switch($accion) {
    ...
    case 'obtener_entregas':
        // Obtener reservas aprobadas para entregar
        $sql = "
            SELECT
                reservas.id,
    ...
    case 'registrar_entrega':
        ...
        try {
            ...
            // 1. Cambiar estado de reserva a 'entregado'
            $sql_reserva = "UPDATE reservas SET estado = 'entregado' WHERE id = ?";
            $stmt_reserva = $pdo->prepare($sql_reserva);
            $stmt_reserva->execute([$reserva_id]);

            // 2. Cambiar estado de videobeam a 'prestado'
            $sql_videobeam = "UPDATE videobeams SET estado = 'prestado' WHERE id = ?";
            ...
            echo json_encode([
                "ok" => false,
                "mensaje" => "Error al registrar entrega: " . $e->getMessage()
            ]);
        }
